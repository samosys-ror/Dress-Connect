...
-(void)InsertEvent:(SEL)tempSelector tempTarget:(id)tempTarget :(NSString *)user_id :(NSString *)title :(NSString *)location :(NSString *)dressCode :(NSString *)date : (NSString *)month
{
    //http://103.15.67.74/pro1/dressconnect/webservice/dressconnect_webservice.php?method=insert_event&userid=1&title=abc&location=aaaa&dresscode=any&date=23-02-2016&month=february
...
//        //parameter6
        [body appendData:[[NSString stringWithFormat:@"\r\n--%@\r\n",boundary] dataUsingEncoding:NSUTF8StringEncoding]];
        [body appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"month\"\r\n\r\n%@", month] dataUsingEncoding:NSUTF8StringEncoding]];
        [body appendData:[[NSString stringWithFormat:@"\r\n--%@\r\n",boundary] dataUsingEncoding:NSUTF8StringEncoding]];
...
//    }
//}
-(void)show_event:(SEL)tempSelector tempTarget:(id)tempTarget : (NSString *)user_id
{
    http://103.15.67.74/pro1/dressconnect/webservice/dressconnect_webservice.php?method=show_event&userid=1
    l_appDelegate=(AppDelegate *)[[UIApplication sharedApplication]delegate];
    ...
    if([Utilities CheckInternetConnection])//0: internet working
    {
        [UIApplication sharedApplication].networkActivityIndicatorVisible = YES;
        NSMutableString *temp_url;
        temp_url=[NSMutableString stringWithFormat:@"%@new_show_event&userid=%@",WEBURL,user_id];
        temp_url=(NSMutableString *)[temp_url stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        NSLog(@"%@",temp_url);
